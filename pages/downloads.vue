<template>
  <div class="container py-4 content">
    ...
    <div v-else>
      ...
      <template v-if="descargasFiltradas.length > 0">
        <div
          v-for="(grupoDescargas, so) in descargasPorSO"
          :key="so"
          class="mb-5"
        >
          ...
          <div class="row g-4">
            <div
              v-for="descarga in grupoDescargas"
              :key="descarga.id"
                ...
                >
                  <div class="d-flex align-items-center">
                    <i
                      :class="[
                        'bi',
                        iconoPorSO(descarga.sistema_operativo),
                        'me-2 text-muted',
                      ]"
                    ></i>
                  ...
                  <div class="d-flex gap-2">
                    <span class="win-btn win-minimize"
                      ><i class="bi bi-dash"></i
                    ></span>
                    <span class="win-btn win-maximize"
                      ><i class="bi bi-square"></i
                    ></span>
                    <span class="win-btn win-close"
                      ><i class="bi bi-x"></i
                    ></span>
                  </div>
                </div>
...
<<<<<<< HEAD
    ...
    if (result.error) {
      const errMessage = String(result.error.message || result.error);
      console.warn(
        "Supabase descargas query failed, retrying without order:",
        errMessage
      );
      result = await supabase.from("descargas").select("*");
...
>>>>>>> 65f12d4795b9284c5cf986ae2453462f7b064921
    ...
    error.value =
      err instanceof Error ? err.message : "Error de Conexion, Verifique e intente de nuevo";
  } finally {
...
const descargasFiltradas = computed<Descarga[]>(() => {
  ...
  return descargas.value.filter((descarga) => {
    ...
    const coincideAsignatura =
      ...
      descarga.asignatura === filtroAsignatura.value;
...
function toggleAsignatura(asignatura: string) {
  filtroAsignatura.value =
    filtroAsignatura.value === asignatura ? "" : asignatura;
}
